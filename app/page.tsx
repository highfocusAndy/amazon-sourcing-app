...
// ── Hero ──────────────────────────────────────────────────────────────────────
// CHANGE 1: shortened subheadline + added gold CTA before verdict tags
// CHANGE 3: vertical spacing reduced 25% (pb-20→pb-16, pt-16→pt-12, mt-8→mt-6, mt-14→mt-10)
function HeroSection() {
  return (
    <section
      className="lp-grid-bg relative flex min-h-[92vh] flex-col items-center justify-center overflow-hidden px-6 pb-16 pt-12 text-center"
    >
      {/* Floating orbs */}
      <div
      ...
      <div className="relative z-10 mx-auto max-w-5xl">
        ...
        {/* CHANGE 1: shortened subheadline */}
        <p
          className="lp-b mx-auto mt-6 max-w-2xl leading-relaxed text-slate-400 text-[0.95rem] sm:text-[1.075rem]"
        >
          Get live Buy Box prices, FBA fees, and instant{" "}
          <span className="font-semibold text-slate-200">BUY · PASS · WORTH UNGATING</span>{" "}
          verdicts in seconds. No spreadsheets. No guesswork.
        </p>

        {/* CHANGE 1: gold "Start Free Trial →" button, immediately after paragraph, before verdict tags */}
        <div className="mt-8 flex justify-center">
          <Link
            href="#pricing"
            className="lp-btn-g lp-b inline-flex items-center gap-2 rounded-xl px-7 py-3.5 text-sm font-bold text-black shadow-lg"
            style={{ boxShadow: "0 8px 30px -8px rgba(201,168,76,0.45)" }}
          >
            Start Free Trial →
          </Link>
        </div>

        {/* CHANGE 3: mt-14 → mt-10 */}
        {/* Decision badges */}
...
// ── Stats bar ─────────────────────────────────────────────────────────────────
// CHANGE 3: py-7 → py-5, gap-y-7 → gap-y-5
function StatsBar() {
  return (
    <div className="lp-b border-y px-6 py-5" style={{ borderColor: C_BORD, background: "rgba(255,255,255,0.015)" }}>
      <div className="mx-auto grid max-w-4xl grid-cols-2 gap-y-5 sm:grid-cols-4">
        {[
...
// ── Features ──────────────────────────────────────────────────────────────────
// CHANGE 2: card border changed to 1px solid rgba(201,168,76,0.25)
// CHANGE 3: py-24 → py-[4.5rem], mb-14 → mb-10
    ...
    <section className="px-6 py-[4.5rem]">
      <div className="mx-auto max-w-6xl">
        <ScrollReveal>
          ...
          <p className="lp-b mx-auto mb-10 max-w-lg text-center text-slate-500">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map(({ n, icon, title, desc, pro }, i) => (
            <ScrollReveal key={n} delay={i * 65}>
              {/* CHANGE 2: borderColor → rgba(201,168,76,0.25) */}
              <div
                className="lp-feat lp-b flex h-full flex-col rounded-2xl border p-7"
                style={{ background: CARD, borderColor: "rgba(201,168,76,0.25)" }}
              >
...
// ── How It Works ──────────────────────────────────────────────────────────────
// CHANGE 3: py-24→py-[4.5rem], mb-20→mb-[3.75rem], mt-20→mt-[3.75rem], mb-16→mb-12, gap-14→gap-10
function HowItWorksSection() {
  ...
  return (
    <section className="px-6 py-[4.5rem]">
      <div className="mx-auto max-w-6xl">
        <div className="lp-divider mb-[3.75rem]" />

        <ScrollReveal>
          <Label>How It Works</Label>
          <h2
            className="lp-h mb-12 text-center text-white"
            style={{ fontSize: "clamp(2rem, 5vw, 3.2rem)", fontStyle: "italic", fontWeight: 600 }}
          >
        ...
        <div className="lp-divider mt-[3.75rem]" />
      </div>
    </section>
  );
}
...
// ── Testimonials ──────────────────────────────────────────────────────────────
// CHANGE 4: new section added after How It Works
const testimonials = [
  {
    name: "Marcus T.",
    role: "Amazon FBA Seller — 3 years",
    text: "I used to spend 2 hours manually checking each product. Now I get through a 50-item supplier list in under 20 minutes. The BUY/PASS verdicts alone are worth it.",
  },
  {
    name: "Sarah K.",
    role: "Wholesale Sourcing Coach",
    text: "I recommend this to every student in my course. The restriction checks and ungating calculator save them from making costly mistakes with new brands.",
  },
  {
    name: "Daniel R.",
    role: "FBA Reseller — 5+ years",
    text: "The real-time Buy Box data matches what I see in Seller Central. I trust these numbers when I'm making buying decisions — that accuracy is everything.",
  },
];

function TestimonialsSection() {
  return (
    <section className="px-6 py-[4.5rem]" style={{ background: "rgba(255,255,255,0.012)" }}>
      <div className="mx-auto max-w-6xl">
        <ScrollReveal>
          <Label>What Sellers Are Saying</Label>
          <h2
            className="lp-h mb-10 text-center text-white"
            style={{ fontSize: "clamp(2rem, 5vw, 3.2rem)", fontStyle: "italic", fontWeight: 600 }}
          >
            Trusted by wholesale resellers
          </h2>
        </ScrollReveal>
          ...
          {testimonials.map(({ name, role, text }, i) => (
            <ScrollReveal key={name} delay={i * 90}>
              <div
                ...
                style={{ background: CARD, border: `1px solid ${G_BORD}` }}
              >
                {/* Gold stars */}
                <div className="mb-4 flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, si) => (
                    <svg key={si} className="h-4 w-4 shrink-0" viewBox="0 0 20 20" aria-hidden>
                      <path
                        fill={G}
                        d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.368 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.368-2.447a1 1 0 00-1.175 0l-3.368 2.447c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.062 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69L9.049 2.927z"
                      />
                    </svg>
                  ))}
                </div>

                <p className="lp-b flex-1 text-[13.5px] leading-relaxed text-slate-400">
                  &ldquo;{text}&rdquo;
                </p>
                ...
                >
                  <p className="lp-b text-sm font-semibold text-white">{name}</p>
                  <p className="lp-b text-[11px] text-slate-600">{role}</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
...
function LandingFooter() {
  return (
    <footer className="lp-b border-t px-6 py-8" style={{ borderColor: C_BORD }}>
...
export default async function HomePage() {
  ...
  return (
    ...
    >
      ...
      <FeaturesSection />
      <HowItWorksSection />
      {/* CHANGE 4: testimonials after How It Works */}
      <TestimonialsSection />
      ...
      />
      {/* CHANGE 5: FAQ after pricing, before footer */}
      <LandingFAQ />
      <LandingFooter />
    </div>
  );
